...
Here I include a full image, with 3024 singular values, and the low-rank approximation of the image, keeping only 100 singular values. The second image, while inferior in quality, requires less than 6\% of the storage of the first.
...
Twilight picture (and made black-and-white) of my neighbors' cat, LB, in full detail (top); and in a low-rank approximation (bottom). The original image is $3024\times 4032$ pixels, meaning the matrix describing it has 12.2 million entries, the intensity of each pixel, and 3024 singular values. The full SVD has $3024\times (3024+4032)=21.3$ million numbers. However, in the second image, we keep only 100 of the singular values, which means we need just $100\times (3024+4032)=706,000$ numbers to describe the image. We still get a lot of of detail of LB, of the grain in the wood, of the trees, the pine needles, etc. Comparing against the original size, that is ostensibly $<6\%$ the space. That is, the low-rank approximation does a really good job replicating the original image in just $\sim 1/20^{th}$ the space. (Full disclosure, the images I saved are 2.8~MB and 2.3~MB, signaling that this isn't how MATLAB actually saves the images.)
```
