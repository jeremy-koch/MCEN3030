...
In your linear algebra course you probably spent some time calculating these things for a given matrix by creating a polynomial from the determinant: $\left|\mathbf{A}-\lambda \mathbf{I}\right|$ (where $\mathbf{I}$ is the identity matrix) and figuring out its roots. This is definitely NOT how eigenvalues are calculated for large systems. It is kind of a disservice that the typical linear algebra class had students finding eigenvalues of a $2\times 2$ matrix by hand from the polynomial. That whole exercise carries very little meaning, delivers no intuition, and is not even how eigenvalues are calculated in most practical situations. Instead, the eigenvalues are usually determined via an iterative process based on QR-decomposition... but that is too much math to talk about here.
:::{aside}
A $5\times 5$ matrix is not even ``large'', yet solving the fifth-degree polynomial is impossible for a human and not straightforward for a computer either. In many applications, the matrix is even larger, maybe much larger, and there is no hope of finding the polynomial roots!
:::

## The Decomposition
An $n \times n$ matrix (of full rank) will have $n$ eigenvectors. If we gather them together as columns of a matrix (called $\mathbf{V}$) and include the eigenvalues in a diagonal matrix (called $\boldsymbol{\Lambda}$), we could\footnote{where we use the order $\mathbf{V}\boldsymbol{\Lambda}$ instead of $\boldsymbol{\Lambda}\mathbf{V}$ to make sure each column of $\mathbf{V}$ is multiplied by the correct $\lambda$.} write $\mathbf{A}\mathbf{V}=\mathbf{V}\boldsymbol{\Lambda}$. Or, writing in the typical decomposed form:

$$
    \mathbf{A}=\mathbf{V}\boldsymbol{\Lambda}\mathbf{V}^{-1}.
$$

This is the EVD of matrix $\mathbf{A}$. MATLAB has a built-in implementation, \verb|[V,L]=eig(A)|. The outputs are $\mathbf{V}$, a matrix whose columns are the eigenvectors of $\mathbf{A}$, and $\mathbf{L}$, a diagonal matrix of eigenvalues.\footnote{When we aren't limited in our character choice and can use Greek symbols, we choose $\boldsymbol{\Lambda}$, a capital $\lambda$, instead of $\mathbf{L}$.}
...
Let's examine the discrete system\footnote{This system represents this situation: What are the populations of $x$ and $y$ at the next time step, $i+1$, as a function of the populations at the current time step, $i$. It isn't a very meaningful predator-prey model -- those usually have a nonlinear term $\dot{x}\sim -xy$ that implies a consumption rate that increases with both the population of predator and prey. A nonlinear system can't immediately be put into a matrix framework, but can be linearized around critical points and then matrixified to understand those critical points.}
...
The eigen-decomposition of this matrix is\footnote{Determined via MATLAB's [V,L]=eig(A)}
...
So, one positive eigenvalue, one negative eigenvalue. Starting with populations of $x_0=0.5$, $y_0=0.5$ (this is arbitrary), after 1000 generations we have... well, a big value for each, but the more interesting thing is the ratio:

$$
    \frac{x_{1000}}{y_{1000}}=2.618=\frac{0.9342}{0.3568}.
$$

That is, the "vector" $[x,y]^T$ ends up pointing in the same direction as the eigenvector associated with the biggest eigenvalue. This provides some intuition about a meaning of the eigenvectors: when a matrix is applied to a vector, the vector will be stretched in the direction of the eigenvector with the greatest eigenvalue. Here, that has meaning: This ends up being the steady-state population ratio between species $x$ and $y$.\footnote{Again, a better population model would include some effect of overpopulation that would keep the populations from going to $\infty$. The example we did in class had this feature -- the populations were cyclical and bounded, and were mathematically chaotic!}
...
\footnote{The one we did in-class:
    ...
    \end{bmatrix}_2
\end{equation*}
where the columns were interpreted as the fraction of links on a given site that led to another site. Thus, the zeros in the diagonal. E.g., looking at the last column: if you are on website D and it has two links, one goes to website A and the other to C, so the chance of going to A is 50\% and C is 50\%.}

We can imagine an internet user marching their way through the links on various webpages, which would correspond to repeatedly multiplying the population vector $\mathbf{x}$ by the matrix. And we might wonder: will this ever settle into a steady-state distribution, independent of the starting point, such that applying the matrix just gets us the same distribution again? I.e., $\mathbf{x}_i=\mathbf{x}_{i+1}$. Sounds like an eigenvalue problem -- the distribution we are describing would be an eigenvector with an associated eigenvalue of 1. For the system described at the right, the eigenvector associated with $\lambda=1$ is
\begin{equation*}
    \mathbf{x}=
    \begin{bmatrix}
...
:::{aside}
What if there are two "subcultures" that reference a search term that don't reference each other at all? Then, depending on the starting population $\mathbf{x}$, a user might never find the second subculture. Page and Brin included the probability of a a random restart point that would then have a chance throw the searcher to the other sites. Check out Raluca Tanase \& Remus Radu's https://pi.math.cornell.edu/~mec/Winter2009/RalucaRemus/index.html for more details of the math.
:::
