...
This course is less about using built-in functions like ```fsolve``` and more about understanding the algorithms. Truthfully, nearly all of the necessary coding elements to write all the functions in this course appear on this page -- indexing, ```if```, ```for```, ```while```. You'll need to do some space preallocation, might need to use the modulus function... not much else.  
