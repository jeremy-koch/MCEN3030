...
If I'm being honest, I am not sure the practicalities of how the SVD is calculated numerically. But we can still try to develop some intuition about what is going on, and talking about the {abbr}'matrix rank (the number of linearly independent columns or rows)` is one way to do that. {abbr}`MyST (Markedly Structured Text)`
