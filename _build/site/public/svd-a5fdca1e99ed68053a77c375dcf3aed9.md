...
Actually: we can can get a really decent image with the low-rank approximation, wherein we essentially discard a bunch of information, thus reducing the storage size. And SVD is the tool that we use to do that. Here I include a full image, with 3024 singular values, and the low-rank approximation of the image, keeping only 100 singular values. The second image, while inferior in quality, requires less than 6\% of the storage of the first.
