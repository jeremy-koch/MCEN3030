...
This rationalization lends itself well to understanding why SVD works in image processing. A black-and-white image can be interpreted as a matrix, with the value in each entry the intensity of that pixel. To simplify the discussion, let's say the image/matrix is square, with $m$ rows and $m$ columns. So the matrix rank is $m$, and there are $m^2$ numbers stored in the matrix. The decomposition to $\mathbf{A}=\boldsymbol{\alpha}\mathbf{a}^T+\boldsymbol{\beta}\mathbf{b}^T+\boldsymbol{\gamma}\mathbf{c}^T+ \dots$ requires storage of $m(m+m)=2m^2$ numbers. So the SVD is a less-efficient way to store data?! 

Actually: we can can get a really decent image with the low-rank approximation, wherein we essentially discard a bunch of information, thus reducing the storage size. And SVD is the tool that we use to do that.
