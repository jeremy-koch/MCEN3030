...
This is an introductory course, and arguably it is an "applied math" course. We aren't going to reinvent SolidWorks this semester, but we will learn about stiffness matrices and matrix inversion, which is ostensibly what is going on under the hood in SolidWorks. We also are not writing detailed software packages with fancy user interfaces -- it is going to be mostly just the math, implemented with basic programming ideas like array indexing, for loops, while loops, etc. The hope is that, by examining the fundamentals of these ideas, you will have some perspective on their power and limitations when it comes time to use these tools. Further, you will be ready to tackle more detailed/complicated ideas, if you decide to continue your studies of computational methods in advanced courses.

Our focus this semester is less about using the tools and more about understanding the tools. We will build our own versions of things like differential equation solvers, and that will allow us to better understand: what information is needed, what errors might we see, what can we do if they are not working, ... .
...
- Course units are in the panel on the left, and clicking them will open a drop-down menu. Each item in that drop-down menu is a short reading. The readings are mostly about the motivation and the math, though some code is included.
...
That being said, this webpage is hopefully going to be a resource for you, and you should read the sections I point out before class and reference this first when you have questions (use the search bar!). I estimate the total word count on all these pages to be about 30000... which sounds like a lot, but a textbook has about 300 words per page, so this is something like 100 pages. Over the course of our 15 weeks together, it is incredibly reasonable to ask you to read and digest this volume of information -- equivalent to 7ish pages per week!
...
- Passivity is getting you nowhere. Think about the material after you read a section, get a pencil out and derive something yourself, do some digging, get curious ("why doesn't this work?"), connect the dots.

One of my big goals this semester is to empower students instead of 