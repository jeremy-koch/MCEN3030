...
In this course, you have the green light to use {abbr}`AI (artificial intelligence)`/{abbr}`LLMs (large language models)` on homework (not on exams or in-class problems, which are pencil and paper). Here, I will give a perspective on AI misuse and also will give recommendations on how it can be properly used.
