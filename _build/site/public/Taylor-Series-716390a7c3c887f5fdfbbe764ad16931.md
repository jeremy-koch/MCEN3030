...
\begin{align}
\hat{f}(x) &= f(x_0)+ f'(x_0)(x-x_0) + \frac{1}{2!}f''(x_0)(x-x_0)^2 + \frac{1}{3!}f'''(x_0)(x-x_0)^3 \dots \\
& = f(x_0) + \sum\limits_{n=1}^\infty \frac{1}{n!}f^{(n)}\bigg\rvert_{x_0}(x-x_0)^n
\end{align}
out to infinity terms. Importantly, this equivalent function, $\hat{f}(x)$, is a polynomial, and polynomials are easy to deal with! The trade-off, of course, is that there are now infinity terms. We'll see if we can tame that.

## Truncation Error

In practice, we (either by hand our with a computer) cannot process infinity terms, and so we must "truncate" the summation 
...
\begin{equation}
\text{error} = f(x) - \hat{f}(x) = f(x) - \left[f(x_0)+ f'(x_0)(x-x_0) + \dots \right]
\end{equation}
