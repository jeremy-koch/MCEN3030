...
- Indexing starts with zero, which might annoy you enough to look elsewhere.
- There are plenty of tutorials available, but you must install .
- Base Python was not invented for engineers. NumPy/SciPy/Matplotlib have been thoroughly developed, but it just less appealing to have to deal with ```np.sin(x)``` instead of just ```sin(x)```. (Yes, I know you can ```from numpy import sin``` but that is not all that would need importing. We usually just ```import numpy``` in its entirety.)
:::
...
- Julia is a compiled language. This means the first time you run it, it may be slow. If you are the type to make an incremental change, then run it to check the results, you may be frustrated by the slowness.
...
- It has arguably been the language of "serious engineers" for the past 30 years. 

Reasons to NOT choose C++:
- C++ is a compiled language.
- I think it is the ugliest-looking and least-readable language of the four in the list. 
:::
