...
```
The original image is $3024\times 4032$ pixels, meaning the matrix describing it has 12.2 million entries (the intensity of each pixel) and 3024 singular values. The full SVD has $3024\times (3024+4032)=21.3$ million numbers.

However, the singular values are ranked, and many of them are nearly zero. Effectively, we toss out the ones that are nearly zero, and then write $\mathbf{U}$, $\mathbf{S}$, and $\mathbf{V}$ in a reduced form. After recombining them, the image is inferior, but not bad, and requires much less space to store.
