...
But what about 
$$\frac{d^2 x}{dt^2} -\mu (1-x^2)\frac{dx}{dt}+x=F(t)?$$
Hint: there is no analytical solution. But it comes up in [vacuum tube circuits](https://en.wikipedia.org/wiki/Van_der_Pol_oscillator)!


This is an introductory course -- we aren't going to reinvent SolidWorks this semester, but we will learn about . The hope is that, by examining the fundamentals of these ideas, you will have some perspective on their power and limitations when it comes time to use something like a SolidWorks.

A quick example of where you might take your wisdom after completing MCEN 3030: I stumbled upon [Agents.jl](https://juliadynamics.github.io/AgentsExampleZoo.jl/dev/examples/forest_fire/), which is a Julia package that can implement flocking/swarming models, and one of their applications was to model forest fires -- super relevant to us in Colorado!
...
A simulation of forest fire propagation, from [Agents.jl](https://juliadynamics.github.io/AgentsExampleZoo.jl/dev/examples/forest_fire/)
```
...
I prepared this site to augment the learning . It is a living document that will have things added and removed frequently. As such, I do NOT recommend forking this repository (if you know how to do this) -- if you are reading this early in the semester, there might be 50% more material by the end of the semester.
