...
This is an introductory course with a fair amount of coding, though it is arguably more of an "applied math" course than a programming course, with most of the heavy lifting been done with ```for```, ```while```, ```if```, and thoughtful use of arrays. We aren't going to reinvent SolidWorks this semester, but we will learn about stiffness matrices and matrix inversion, which is ostensibly what is going on under the hood in SolidWorks.
