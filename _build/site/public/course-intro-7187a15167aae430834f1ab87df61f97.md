...
In this course we will learn about many of the mathematical algorithms that power engineering calculations. This is an exciting topic, as it allows us to understand problems that go beyond what we can do with pencil and paper. Your math courses might have had you solving $\dot{x}(t) = x\cos(t)$... $x(t) = c_1 \exp(\sin t)$. But what about $\frac{d^2 x}{dt^2} -\mu (1-x^2)\frac{dx}{dt}+x=F(t)$? Hint: there is no analytical solution. But it comes up in [vacuum tube circuits](https://en.wikipedia.org/wiki/Van_der_Pol_oscillator)!
