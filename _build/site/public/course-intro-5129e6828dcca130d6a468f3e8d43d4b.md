...
- Because this is a "polyglot" course, most of the times that code is referenced, it will be included in an environment like that below: choose your language, then see the code in that language. In the top-right corner of each coding environment, you can choose an icon to copy that code block. While we can't run the code on the webpage, you can paste it into your working environment and run it. Please understand that it is a big undertaking to include this feature, and there may be some small mistakes in the code! Confirm it does what you think it does!
::::{tab-set}
:::{tab-item} MATLAB
```matlab
f=@(x) sin(x);
